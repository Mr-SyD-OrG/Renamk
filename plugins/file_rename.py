...
async def refunc(client, message):
    ...
    syd = file.file_name
    filename = ' '.join(filter(lambda x: not x.startswith('PAHE') and not x.startswith('PrimeFix') and not x.startswith('HDA') and not x.startswith('PSA') and not x.startswith('PAHE') and not x.startswith('GalaxyRG') and not x.startswith('-Bigil') and not x.startswith('TR') and not x.startswith('[') and not x.startswith('www.') and (not x.startswith('@') or x == '@GetTGLinks'), file.file_name.split()))
    filesize = humanize.naturalsize(file.file_size)
    sydd = ['psa', 'bigil', 'budgetbits', 'HDA', 'TR', 'primefix', 'GalaxyRG265', 'bone', 'Incursi0', 'StreliziA', 'ikaRos', 'lssjbroly', 'soan', 'pahe', 'poke', 'galaxytv', 'galaxyrg']
